...
use rayon_adaptive::{
    fuse_slices, Divisible, DivisibleAtIndex, EdibleSlice, EdibleSliceMut, Mergeable, Policy,
};
use std::iter::repeat;
...
fn main() {
    let mut v: Vec<u32> = (0..10_000_000).rev().collect();
    let sorted_v: Vec<u32> = (0..10_000_000).collect();
    adaptive_sort(&mut v);
