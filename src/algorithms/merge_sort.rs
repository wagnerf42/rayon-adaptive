...
// PG-13
fn clone_slice<'a, T: 'a>(some_slice: &mut &'a mut [T]) -> &'a mut [T] {
    unsafe { from_raw_parts_mut(some_slice.as_mut_ptr(), some_slice.len()) }
}
        ...
        .reduce(
            ...
            |mut left_sorted, mut right_sorted| {
                if left_sorted.inp_slice.is_none() {
                    ...
                    let mut left_input = left_sorted.inp_slice.take().unwrap();
                    let mut right_input = right_sorted.inp_slice.take().unwrap();
                    ...
                    (&mut new_output)
                        .into_iter()
                        .zip(merge(
                            clone_slice(&mut left_input),
                            clone_slice(&mut right_input),
                        ))
                        .for_each(|(outp, inp)| {
                            *outp = *inp;
                        });
