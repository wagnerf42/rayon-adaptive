...
pub struct ParallelMerge<I, J, S> {
    pub(crate) i: DivisibleIter<I, S>,
    pub(crate) j: DivisibleIter<J, S>,
}

struct LeftLeft<I, J, S> {
    i: DivisibleIter<I, S>,
    j: DivisibleIter<J, S>,
}

struct RightRight<'niq, I, J, S> {
    i: DislocatedMut<'niq, DivisibleIter<I, S>>,
    j: DislocatedMut<'niq, DivisibleIter<J, S>>,
}

pub struct BorrowingParallelMerge<'par, I, J, S> {
    ij: either::Either<LeftLeft<I, J, S>, RightRight<'par, I, J, S>>,
    size: usize,
}

impl<I, J, S> ItemProducer for ParallelMerge<I, J, S>
where
    I: IntoIterator,
    I::Item: Send,
{
...
impl<'par, I, J, S> ItemProducer for BorrowingParallelMerge<'par, I, J, S>
where
    I: IntoIterator,
    I::Item: Send,
{
...
impl<I, J, S> Powered for ParallelMerge<I, J, S> {
    type Power = Indexed;
}

impl<'par, I, J, S: Copy + Schedulable + Send + Sync> ParBorrowed<'par> for ParallelMerge<I, J, S>
where
    I: DivisibleParallelIterator + IntoIterator + Sync,
    DivisibleIter<I, S>: Index<usize>,
    <DivisibleIter<I, S> as Index<usize>>::Output: Ord + Sized,
    I::Item: Send,
    J: DivisibleParallelIterator + IntoIterator<Item = I::Item> + Sync,
    DivisibleIter<J, S>: Index<usize, Output = <DivisibleIter<I, S> as Index<usize>>::Output>,
    S: Copy + Schedulable + Send + Sync,
{
    type Iter = BorrowingParallelMerge<'par, I, J, S>;
}

impl<'par, 'seq, I, J, S> SeqBorrowed<'seq> for BorrowingParallelMerge<'par, I, J, S>
where
    I: DivisibleParallelIterator + IntoIterator + Sync,
    DivisibleIter<I, S>: Index<usize>,
    <DivisibleIter<I, S> as Index<usize>>::Output: Ord,
    I::Item: Send,
    J: DivisibleParallelIterator + IntoIterator<Item = I::Item> + Sync,
    DivisibleIter<J, S>: Index<usize, Output = <DivisibleIter<I, S> as Index<usize>>::Output>,
    S: Copy + Schedulable + Send + Sync,
{
    type Iter = Take<SequentialMerge<'seq, I, J, S>>;
}

impl<I, J, S> ParallelIterator for ParallelMerge<I, J, S>
where
    I: DivisibleParallelIterator + IntoIterator + Sync,
    DivisibleIter<I, S>: Index<usize>,
    <DivisibleIter<I, S> as Index<usize>>::Output: Ord + Sized,
...
impl<'par, I, J, S> BorrowingParallelIterator for BorrowingParallelMerge<'par, I, J, S>
where
    I: DivisibleParallelIterator + IntoIterator + Sync,
    DivisibleIter<I, S>: Index<usize>,
    <DivisibleIter<I, S> as Index<usize>>::Output: Ord + Sized,
...
fn cut_around_middle<I, S>(sorted_iter: &mut DivisibleIter<I, S>) -> I
where
    I: DivisibleParallelIterator + IntoIterator + Sync,
    DivisibleIter<I, S>: Index<usize>,
    <DivisibleIter<I, S> as Index<usize>>::Output: Ord + Sized,
...
fn search_and_cut<I, S>(
    sorted_iter: &mut DivisibleIter<I, S>,
    value: &<DivisibleIter<I, S> as Index<usize>>::Output,
) -> I
where
    I: DivisibleParallelIterator + IntoIterator + Sync,
    DivisibleIter<I, S>: Index<usize>,
...
impl<'par, I, J, S> Divisible for BorrowingParallelMerge<'par, I, J, S>
where
    I: DivisibleParallelIterator + IntoIterator + Sync,
    DivisibleIter<I, S>: Index<usize>,
    <DivisibleIter<I, S> as Index<usize>>::Output: Ord + Sized,
    ...
    fn divide(mut self) -> (Self, Self) {
        ...
        let (left_i_diviter, left_j_diviter) = if i_len < j_len {
            ...
            let left_j_diviter = DivisibleIter {
                schedule_type: j_iter.schedule_type,
            ...
            let left_i_diviter = DivisibleIter {
                schedule_type: i_iter.schedule_type,
        ...
        let left_div: either::Either<LeftLeft<I, J, S>, RightRight<I, J, S>> =
...
pub struct SequentialMerge<'e, I, J, S> {
    i: DislocatedMut<'e, DivisibleIter<I, S>>,
    j: DislocatedMut<'e, DivisibleIter<J, S>>,
}

impl<'e, I, J, S> Iterator for SequentialMerge<'e, I, J, S>
where
    I: DivisibleParallelIterator + IntoIterator + Sync,
    DivisibleIter<I, S>: Index<usize>,
    <DivisibleIter<I, S> as Index<usize>>::Output: Ord,
