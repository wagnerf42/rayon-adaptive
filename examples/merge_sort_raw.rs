...
use rayon_adaptive::{BasicPower, IndexedPower, Policy};
use std;
use std::iter::repeat;
...
impl<'a, T: 'a + Send + Sync + Ord + Copy> Divisible for FusionSlice<'a, T> {
    ...
    fn base_length(&self) -> Option<usize> {
        Some(self.output.len() - self.output_index)
    }
            ...
            && left.s[left.i].last() <= right.s[right.i].first()
        {
            ...
            let destination_index = (0..3).find(|&x| x != left.i && x != right.i).unwrap();
            {
                ...
                if left_input.last() <= right_input.first() {
                    output_slice[..left_input.base_length().unwrap()].copy_from_slice(left_input);
                    output_slice[left_input.base_length().unwrap()..].copy_from_slice(right_input);
                } else if right_input.last() < left_input.first() {
                    output_slice[..right_input.base_length().unwrap()].copy_from_slice(right_input);
                    output_slice[right_input.base_length().unwrap()..].copy_from_slice(left_input);
                } else {
...
pub fn adaptive_sort_raw<T: Ord + Copy + Send + Sync>(slice: &mut [T]) {
    ...
    let mut result_slices = slices
        .cut()
        .with_policy(Policy::Join(slice_len / (num_threads * 2)))
        .map(|mut slices| {
        ...
        .reduce(
            ...
            |o1, o2| {
                if let Some(s1) = o1 {
                    Some(s1.fuse_with_policy(o2.unwrap(), Policy::Adaptive(1_000, 10_000)))
                } else {
                    o2
                }
            },
